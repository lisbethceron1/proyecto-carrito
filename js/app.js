/*const compras= document.querySelector('#lista-cursos')
const agregar= document.querySelector('#agregados')
let tweets= [];
EventListeners();
...
function agregarTweet() {
    ...
    const instructor = document.querySelector("p");
  ...
  function crearHTML() {
    ...
    if (tweets.length > 0) {
        tweets.forEach((titulo) => {
      

            const li = document.createElement('li');
            if (titulo.titulo !== '') {
                const tweetElement = document.createElement('p');
                tweetElement.textContent= titulo.titulo
              
                li.appendChild(tweetElement);
            }
           
            
            agregar.appendChild(li);
        });
    }
